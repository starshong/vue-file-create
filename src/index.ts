...
function toPascalCase(str: string): string {
  return str.split(/\\|\//)
    .reduce((prev: string, curr: string) => {
      const prevPart = prev ? (prev.charAt(0).toUpperCase() + prev.slice(1)) : prev
      const currPart = curr.charAt(0).toUpperCase() + curr.slice(1)
      return prevPart + currPart
    })
}

function generateVueContent(template: string, fileName: string, useTs: boolean): string {
...
function generateRouteContent(template: string, fileName: string): string {
  const s = new MagicString(template)
